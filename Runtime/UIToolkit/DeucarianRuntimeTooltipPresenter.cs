        ...
        private void PositionBubble()
        {
            ...
            if (rootWidth > 0f)
            {
                left = Mathf.Clamp(
                    ...
                    Mathf.Max(
                        EdgeInset,
                        rootWidth - bubbleWidth - EdgeInset));
            }

            if (rootHeight > 0f &&
                top + bubbleHeight + EdgeInset > rootHeight)
            {
                Rect bounds = pendingTarget != null
                    ? pendingTarget.worldBound
                    : default;
                top = RootPosition(
                          new Vector2(bounds.xMin, bounds.yMin)).y -
                      bubbleHeight -
                      9f;
            }
            ...
            if (rootHeight > 0f)
            {
                top = Mathf.Clamp(
                    ...
                    Mathf.Max(
                        EdgeInset,
                        rootHeight - bubbleHeight - EdgeInset));
            }
